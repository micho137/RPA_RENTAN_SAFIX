...
from src.rentan.core.logging_config import setup_logging
...
def run_pipeline(
    *,
    ...
    """
    Pipeline completo RENTAN:
    - Descarga adjuntos de Outlook
    - Extrae ZIPs
    - Procesa facturas (XML/PDF)
    - Genera JSON
    - (Opcional) agregado por document_id
    - (Opcional) SAFIX
    """
    ...
    agg_by_id_json = settings.invoices_by_id_path
    ...
    extractor = ZipInvoiceExtractor(
        ...
        extract_root=extract_root,          # ✅ NOMBRE CORRECTO
    ...
    extract_res = extractor.process_all(index_csv=index_csv)

    logger.info(
        "Extract done | zips=%s | docs_json=%s | errors=%s",
        extract_res.zips,
        extract_res.docs_json,
        extract_res.errors,
    )
    ...
    if run_safix:
        if not excel_path:
            raise ValueError("run_safix=True pero no se recibió excel_path")
